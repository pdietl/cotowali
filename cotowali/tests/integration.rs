...
mod cotowali_testutil {
    ...
    pub fn run_source(path: impl AsRef<Path>) -> Output {
        ...
        Output::from(cotowali::run(source).unwrap())
    }
}
...
fn correct_sources() {
    let sources = "sources";
    cotowali_testutil::walk_files(sources, ".li", |name, path| {
        assert_toml_snapshot!(name, cotowali_testutil::run_source(path));
    });
}

#[test]
fn compile_error() {
    cotowali::compile(Source::inline("xx")).expect_err("");
}
